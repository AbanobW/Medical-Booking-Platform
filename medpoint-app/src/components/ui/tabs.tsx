...
// `min-h-8` rather than `h-8`: a variant-scoped height would out-rank any plain
// `h-*` a caller passes, so taller triggers would overflow the list — and with
// `overflow-x-auto` (which forces `overflow-y: auto`) the list would clip their
// focus ring. A floor keeps the default size and still grows with its content.
