...
/** MedPoint nests included relations as `{ data: [...] }`. */
function wireRelationItems<T>(relation: T[] | { data: T[] } | undefined): T[] {
  if (!relation) return [];
  return Array.isArray(relation) ? relation : (relation.data ?? []);
}

function roleOf(wire: WireUser): Role {
  const names = new Set(wireRelationItems(wire.roles).map((role) => role.name));
