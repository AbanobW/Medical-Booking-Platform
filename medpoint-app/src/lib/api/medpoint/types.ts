...
/** Included relations arrive as `{ data: [...] }`, not a bare array. */
export interface WireRelationList<T> {
  data: T[];
}

export interface WireRole extends WireResource {
  type: "Role";
  name: string;
...
export interface WirePermission extends WireResource {
  type: "Permission";
  name: string;
  description?: string | null;
  display_name?: string | null;
...
export interface WireUser extends WireResource {
  ...
  /** Present when `GET /v1/profile?include=roles` is requested. */
  roles?: WireRole[] | WireRelationList<WireRole>;
  /** Present when `GET /v1/profile?include=permissions` is requested. */
  permissions?: WirePermission[] | WireRelationList<WirePermission>;
}
